function(cargo_build)
  cmake_parse_arguments(CARGO "" "NAME;PACKAGE;PWD" "" ${ARGN})
    ...
    if(CARGO_PACKAGE)
        list(APPEND CARGO_ARGS "-p ${CARGO_PACKAGE}")
    endif()

    if(CARGO_PWD)
      file(TO_CMAKE_PATH ${CMAKE_CURRENT_SOURCE_DIR}/${CARGO_PWD} CARGO_PWD)
    else()
      set(CARGO_PWD ${CMAKE_CURRENT_SOURCE_DIR})
    endif()

    add_custom_command(
        OUTPUT ${LIB_FILE}
        COMMAND ${CARGO_ENV_COMMAND} ${CARGO_EXECUTABLE} ARGS ${CARGO_ARGS}
        WORKING_DIRECTORY ${CARGO_PWD}
        DEPENDS ${LIB_SOURCES}
        COMMENT "running cargo")

    add_library(${CARGO_NAME} STATIC IMPORTED GLOBAL)
    set_target_properties(${CARGO_NAME} PROPERTIES IMPORTED_LOCATION ${LIB_FILE})
    add_custom_target(${CARGO_NAME}-target ALL DEPENDS ${LIB_FILE})
    add_dependencies(${CARGO_NAME} ${CARGO_NAME}-target)
endfunction()
